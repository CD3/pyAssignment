...
def test_CLGrader_tests_string_rendering():
    g = CLGrader()
    g.NS.NAME = "Rusty"
    g.NS.PROBLEM = "removing files"
    with g.add_test() as t:
        t.description = "Testing that {NAME} completed the '{PROBLEM}' problem."
        assert (
            t.description
            == "Testing that Rusty completed the 'removing files' problem."
        )
    assert (
        g._tests[-1].description
        == "Testing that Rusty completed the 'removing files' problem."
    )
...
def test_CLGrader_workdir():
    ...
    for i in [2, 3]:
        assert g._tests[i].command_string.startswith(
            "cd " + os.path.join(os.getcwd(), "test")
        )
...
def test_CLGrader_ShellTest_scoring():
    ...
    with t.add_on_fail_test() as ft:
        ...
        ft.command = "pwd"
    t.run()
    assert t.score == pytest.approx(0.5 * 2 / 3)
...
def test_CLGrader_scoring():
    ...
    assert g.score == pytest.approx((1 + 0.5 + 0.25) / 3)
...
def test_CLGrader_setup():
    ...
    assert (
        g._tests[0].command_string
        == 'MSG="grader startup";MSG="${MSG}|test startup";echo ${MSG}'
    )
...
def test_PythonTest():
    ...
    def check(test:PythonTest):
        return test.NS.val == "1"
